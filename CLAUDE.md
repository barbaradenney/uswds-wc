...
- `pnpm run analyze` - Generate Custom Elements Manifest (custom-elements.json)
- `pnpm run generate:docs` - Generate component README.mdx from CEM
- `pnpm run generate:docs:dry-run` - Preview doc generation without writing files
- `pnpm run audit:skipped-tests` - Audit skipped tests with categorization
- `pnpm run audit:anti-patterns` - Detect Lit/Web Component anti-patterns
...
The codebase includes comprehensive AI agent documentation:

- **`AI_USAGE.md`** - Quick reference for AI agents with component tables, usage examples, constraints
- **`.github/copilot-instructions.md`** - GitHub Copilot-specific instructions for Light DOM, Vitest testing
- **`llms.txt`** - Brief LLM discovery file for AI tool integration
- **`custom-elements.json`** - Machine-readable component API (generated by `pnpm run analyze`)
